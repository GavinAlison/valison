...
public class AxisClientHello {
	...
	/**
	 * 方法一： 应用rpc的方式调用 这种方式就等于远程调用， 即通过url定位告诉远程服务器，<br>
	 * 告知方法名称，参数等， 调用远程服务，得到结果。 使用
	 * org.apache.axis2.rpc.client.RPCServiceClient类调用WebService<br>
	 * 
	 * 【注】：
	 * 
	 ...
	 * 第三个参数表示WebService方法的 返回值类型的Class对象，参数类型为Class[]。 如果被调用的WebService方法没有返回值
	 * 应使用 invokeRobust 方法 该方法只有两个参数，它们的含义与invokeBlocking方法的前两个参数的含义相同。
	 * 
	...
	public static void getRPC() throws Exception {
		...
		System.out
				.println(serviceClient.invokeBlocking(qname, obj, classes)[0]);
	}
