...
select a.id                   as validation_requirement_id,
       ...
       (SELECT WM_CONCAT(P.SUBSYSTEM_NAME)
                                  FROM CAAW_SUB_PLAN_REQUIREMENT Q
                                  LEFT JOIN CAAW_VALIDATE_PLAN P
                                    ON P.ID = Q.PLAN_ID
                                 WHERE Q.REQUIREMENT_ID = A.ID
                                   AND P.PARENT_ID = A.PLAN_ID
                                 GROUP BY P.PARENT_ID)as subsystem_name
